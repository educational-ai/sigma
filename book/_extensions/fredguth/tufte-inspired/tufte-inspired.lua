...
local function endTypstBlock(blocks)
  ...
  function Div(el)
    ...
    if el.classes:includes('column-margin') then
      ...
      -- 7.5pt + 0.55em leading: значимо компактнее 8pt при чтении тех же знаков,
      -- даёт длинным callout помещаться в одной marginalia-странице без обрыва.
      local blocks = pandoc.List({
        pandoc.RawBlock('typst', string.format(
          '#margin-note(dx:%s, dy:%s)[#set text(size: 7.5pt); #set par(first-line-indent: 0em, leading: 0.45em)',
          dx, dy))
      })
