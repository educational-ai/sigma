...
::: {.callout-important title="Поиграть: первый порядок vs безградиентные" collapse="false"}
Минимизируем один и тот же стресс MDS двумя способами из одной стартовой точки: градиентным спуском (использует производную) и двухточечной оценкой ZO-GD (только значения функции). Градиент срывается вниз за десятки шагов; безградиентный метод вязнет — и тем безнадёжнее, чем выше размерность. Поднимите `dim` и убедитесь.
:::
