...
До Word2Vec эту идею считали в лоб: сколько раз какие слова
встречаются рядом (матрица совместной встречаемости), а дальше — SVD
этой матрицы и латентно-семантический анализ (LSA, Deerwester et al.,
1990). Но SVD матрицы $|V| \times |V|$ при словаре
$|V| = 100\,000$ — дорого.
...
Модель усредняет векторы контекстных слов и пропускает их через
...
Считать softmax по словарю из $10^5$ слов — тоже дорого.
...
После обучения на большом корпусе (Google News, около 100 миллиардов
слов) у векторов слов появляется структура:
**смысловые отношения кодируются как направления**.
...
переводит результат в женский вариант той же роли.

Выберите слова **a − b + c** и тащите любую точку-слово мышью: параллелограмм аналогии и ближайшее к результату слово перестраиваются сразу.
...
Разность «Париж − Франция» — это направление «столица своей страны»,
и для всех подобных пар оно примерно одно и то же.

::: {.column-margin}

::: {.callout-tip title="Закон сюжета"}
Нейросеть не учили понимать аналогии. Чтобы предсказывать соседей, ей
пришлось сжать язык в пространство, где работает арифметика, — это
следствие дистрибутивной гипотезы в непрерывном пространстве.
:::

:::

Почему именно около 300 измерений? По теореме Джонсона–Линденштрауса $n$ точек
можно уложить в $\mathbb{R}^d$ так, что попарные расстояния сохранятся
с погрешностью $\varepsilon$, если $d = O(\varepsilon^{-2} \log n)$. Для словаря в 3 миллиона слов это
...
(страна, экономика, население). Разность PMI для таких пар
получается примерно одинаковой.

## Визуализация: проекция t-SNE

300-мерные векторы можно спроецировать на плоскость
методом t-SNE (van der Maaten, Hinton, 2008) — он сохраняет
...
(смысловых и грамматических). Доля верных ответов на нём —
около 60–75% у лучших моделей. Аналогии вида
...
предложения. Каждый слой трансформера порождает свой
...
где близость означает близость по смыслу** — осталась
и в BERT, и в GPT.
...
- **t-SNE** — нелинейная проекция, чтобы посмотреть на векторы глазами
